...
export default function SubscriptionPage() {
  ...
  async function loadBilling() {
    ...
    try {
      ...
      if (methods.length) setPaymentMethod(prev => (methods.includes(prev) ? prev : methods[0]))
    } catch (e) {
  ...
  const paymentMethods = billing?.payment_methods || ['stripe_card', 'apple_pay', 'google_pay']
      ...
      <div style={{ maxWidth: 1040, margin: '0 auto', padding: 22 }}>
        ...
        {showCheckoutPanel && selectedPlanData && (
          <div className="theme-card" style={{ padding: 16 }}>
            ...
            <div style={{ fontWeight: 800, marginBottom: 8 }}>Payment methods ({effectiveCountry})</div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
              {paymentMethods.map(method => (
                <button
                  ...
                  style={{
                    border: `1px solid ${paymentMethod === method ? 'var(--sw-accent)' : 'var(--sw-border)'}`,
                    background: paymentMethod === method ? 'var(--sw-accent-soft)' : 'transparent',
                    color: paymentMethod === method ? 'var(--sw-text)' : 'var(--sw-muted)',
                    borderRadius: 8,
                    padding: '8px 10px',
                    cursor: 'pointer',
                    fontSize: 12,
